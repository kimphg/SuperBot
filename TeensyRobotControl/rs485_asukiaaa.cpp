...
Error Central::writeRegisterBy16t(uint8_t deviceAddress,
  ...
  if(isRequireConfirm)return readQuery(deviceAddress, writeFnCode, data, sizeof(data));
  else return 0;
}
...
Error Central::writeRegistersBy16t(uint8_t deviceAddress,
  ...
  uint8_t readData[4];
  if( isRequireConfirm)return readQuery(deviceAddress, writeMultiFnCode, readData, sizeof(readData));
