...
#define INTEGRAL_LEN 200
#define POS_INTEGRAL_LEN 400
float array_yaw[INTEGRAL_LEN];
float array_pos[POS_INTEGRAL_LEN];
...
float getIntegralPos() {
  float res = 0;
  for (int i = 0; i < POS_INTEGRAL_LEN; i++) {
    res += array_pos[i];
  }
  return res / POS_INTEGRAL_LEN;
}
...
void setIntegralPos(float value) {
  ...
  integral_id_pos++;
  if (integral_id_pos >= POS_INTEGRAL_LEN) integral_id_pos = 0;
}
...
void RobotDriver::report_pos_to_PPU() {
  ppu_report[0] = 0xaa;
  ppu_report[1] = 0x55;
  ppu_report[2] = 0xaa;
  ppu_report[3] = 0xF1;
  ...
  x.value = botx;  // 1bit=1mm accuracy
  y.value = boty;  // 1bit=1mm accuracy
  union {
    unsigned short value;
    uint8_t bytes[2];
  } angle;
  float fangle = botangle / 360.0 * 4096.0;
  if (fangle > 4096) fangle -= 4096;
  if (fangle < 0) fangle += 4096;
  angle.value = int(fangle);  //0 đến 360 độ tương ứng 0 giá trị từ 0 đến 4096
                              //gửi x,y
  ppu_report[7] = x.bytes[0];
  ppu_report[8] = x.bytes[1];
  ppu_report[9] = x.bytes[2];
  ppu_report[10] = x.bytes[3];
  ppu_report[11] = y.bytes[0];
  ppu_report[12] = y.bytes[1];
  ppu_report[13] = y.bytes[2];
  ppu_report[14] = y.bytes[3];
  //gửi angle
  ppu_report[15] = angle.bytes[0];
  ...
  uint16_t crc = gen_crc16(ppu_report, 19);
  ppu_report[19] = crc >> 8;
  ppu_report[20] = crc & 0xff;
  S_COMMAND.write(ppu_report, 21);
  // for (int i = 0; i < 21; i++) {Serial.print(ppu_report[i], HEX);Serial.print(' ');}
  // Serial.flush();
}
...
void RobotDriver::processCommand(String command) {
  ...
  if (tokens.size() >= 3) {
    ...
    if (tokens[1].equals("sync")) {
      ...
      if (tokens[1].equals("top_rot") && (tokens.size() >= 3)) {
        float correct_angle = (tokens[2].toFloat());
        diffAngle+=correct_angle;
      }
      ...
      if (tokens[1].equals("home")) {
        ...
        desY = 2000;  //desY+cos(radians(desAngle));
        gotoMode(MODE_MOVE);
        // DPRINTLN(comX);
        // DPRINTLN(comY);
      }
  ...
  if ((commandMessage[0] == 0xaa) && (commandMessage[1] == 0x55) && (commandMessage[2] == 0xaa)) {
    ...
    if ((commandMessage[3] == 0xf2) && (commandMessage[4] == 0x00)) {
      if ((commandMessage[5] == 0x02) && (commandMessage[6] == 0x00)) {
        if (commandMessageI >= 16) {
          ...
          if (bot_mode == MODE_STANDBY) {
            ...
            S_DEBUG.flush();
            gotoMode(MODE_MOVE);
            S_SENSORS.print("$BINCOMACK,");
            S_SENSORS.print(action), S_SENSORS.println(',');
            S_SENSORS.flush();
          }
        }
      } else if ((commandMessage[5] == 0x05) && (commandMessage[6] == 0x00))  // map data
      {
        ...
        reportPPU();
      } else if ((commandMessage[5] == 0x03) && (commandMessage[6] == 0x00)) {
        report_pos_to_PPU();
      } else if ((commandMessage[5] == 0x06) && (commandMessage[6] == 0x00)) {
        if (commandMessage[7] == 0x01) gotoMode(MODE_STANDBY);
      }
...
// lastPosUpdateMillis= curTime;
  ...
  if (angleIMU < -180) angleIMU += 360;
  // float botangleOld = botangle;

  botangle = angleIMU;
  // diffAngle += (imu_data.gyroZ * 0.59);
...
void RobotDriver::DebugReport() {
  ...
  DPRINT("!$ curSpeed L R Li Ro:");
  DPRINTLN(curSpeedL);
  DPRINTLN(curSpeedR);
  DPRINTLN(curSpeedLift);
  DPRINTLN(botRotationSpeed);
  DPRINT("#");
  DPRINT("!$DesMotSpd RLL:");
  DPRINTLN(desMotSpdR);
  DPRINTLN(desMotSpdL);
  DPRINTLN(desLiftSpeed);
  DPRINTLN(desSpeed);
  DPRINT("#");
  DPRINT("!$RotSpd des cur:");
  DPRINTLN(desRotSpd);
  DPRINTLN(imu_data.gyroZ);
  DPRINT("#");
  S_DEBUG.flush();
  S_DEBUG.print('@');
  if (debugCounter == 0) {
    DPRINT("!$PID yaw:");
    DPRINTLN(Kp_yaw * error_yaw);
    DPRINTLN(Ki_yaw * integral_yaw);
    DPRINTLN(Kd_yaw * derivative_yaw);
    DPRINT("#");
    DPRINT("!$PID pos:");
    DPRINTLN(Kp_pos * error_pos);
    DPRINTLN(Ki_pos * integral_pos);
    DPRINTLN(Kd_pos * derivative_pos);
    DPRINT("#");
    DPRINT("!$LiftLevel cur des:");
    DPRINTLN(curLiftStep);
    DPRINTLN(desLiftLevel);
    DPRINT("#");
  }
  if (debugCounter == 1) {
    DPRINT("!$dx,dy,dd,da:");
    ...
    DPRINT("#");
    DPRINT("!$floorMap:");
    DPRINTLN(floorMap.size());
    DPRINTLN(lastFloorTagid);
    DPRINT("#");
  }
  if (debugCounter == 2) {
    DPRINT("!$GyroYaw:");
    DPRINTLN(imu_data.gyroyaw);
    DPRINT("#");
    DPRINT("!$PIDerr YPL:");
    DPRINTLN(error_yaw);
    DPRINTLN(error_pos);
    DPRINTLN(liftAngleError);
    DPRINT("#");
    DPRINT("!$curtime cursyns:");
    DPRINTLN(curTime);
    DPRINTLN(syncLossCount);
    DPRINT("#");
  ...
  while (portSenBus->available()) {
    ...
    if (result == 1)  //camera 1
    {
      ...
      if ((sbus.cambot.tagID > 0) && (sbus.cambot.stable > 2)) {
        ...
        // Serial.print("tag:");
        // Serial.println(sbus.cambot.tagID);
        ...
        if (lastFloorTagid != mapPoint.id) {
          Serial.println("lastFloorTagid change:");
          Serial.println(mapPoint.id);
...
float RobotDriver::calcPIDyaw(float targetAngle)  //PID calculation for yaw loop
{
  ...
  if (predictedLiftStep > (desLiftStep + STEP_PPR * 0.8)) error_yaw -= 360;
  if (predictedLiftStep < (desLiftStep - STEP_PPR * 0.8)) error_yaw += 360;
...
void RobotDriver::liftStabilize()  //stabilize lift angle with microstep motor driver
{
  ...
  liftAngleError = (liftAngle + diffAngle);
  ...
  float liftPID = 8 * (Kp_lift * liftAngleError + Ki_lift * liftAngleErrori + Kd_lift * liftAngleErrord);
  ...
  liftAcc = constrainVal(liftAcc, -0.03, 0.03);
  desLiftSpeed += liftAcc;
  // Serial.println(desLiftSpeed);
  desLiftSpeed = constrainVal(desLiftSpeed, -30, 30);
  stepFreq = abs(STEP_PPR * desLiftSpeed);
  if (stepFreq > 1000000) stepFreq = 1000000;
...
void RobotDriver::sendSyncPacket() {
  ...
  return;
  DPRINT("!$lastSyncSec:");
  DPRINTLN(lastSyncSec);
  DPRINT("#");
  DPRINT("!$paramTable.size:");
  DPRINTLN(paramTable.size());
  DPRINT("#");
  DPRINT("!$GyroConnect:");
  DPRINTLN(imu.isConnected());
  DPRINT("#");
  DPRINT('@');
  S_DEBUG.flush();
  DPRINT("!$Lift Status:");
  DPRINTLN(curLiftStep);
  DPRINTLN(minLiftStep);
  DPRINTLN(desLiftStep);
  DPRINTLN(desLiftSpeed);
  DPRINT("#");
  DPRINT("!$Motor Fail:");
  DPRINTLN(M1ComTime);
...
void RobotDriver::checkMinMax() {
  ...
  if (isLiftMinPos) {
    ...
    desLiftStep = minLiftStep + STEP_PPR * 0.75;
    // liftComm=0;
      ...
      if (reportPacket[0] == 0xaa) {
        if (reportPacket[2] == 0x83)  //lift motor report
        {
          ...
          if (cs == reportPacket[7]) {
            ...
            if (isLiftMaxPos) {
              minLiftStep = curLiftStep - LIFT_PPR * 8;
              desLiftStep = minLiftStep + STEP_PPR * 6.5;
            }
