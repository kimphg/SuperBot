...
float Kp_roll_angle = 0.48;    //Roll P-gain - angle mode
float Ki_roll_angle = 0.25;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.08;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.48;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.25;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.08;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
...
float Kp_yaw = 0.0005;           //Yaw P-gain
...
void controlANGLE() {
  ...
  */
  ...
  error_roll = constrain(error_roll,-20,20);
  ...
  */
  ...
  if (channel_3_pwm < 1100)
  {
    ...
    m6_command_scaled = 0;
      ...
      m2_command_scaled = thro_des + roll_PID + yaw_PID - pitch_PID; //Front Left
      m3_command_scaled = thro_des- roll_PID - yaw_PID - pitch_PID; //Front Right
      m4_command_scaled = thro_des + 0.06 - roll_PID+ yaw_PID + pitch_PID; //Back Right
      m1_command_scaled = thro_des + 0.04 + roll_PID - yaw_PID + pitch_PID; //Back Left
