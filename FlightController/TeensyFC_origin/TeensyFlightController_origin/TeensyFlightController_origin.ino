...
float B_gyro = 0.5;       //Gyro LP filter paramter, (MPU6050 default: 0.1. MPU9250 default: 0.17)
...
float i_limit = 4.0;     //Integrator saturation level, mostly for safety (default 25.0)
float maxRoll = 5.0;     //Max roll angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
float maxPitch = 5.0;    //Max pitch angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
float maxYaw = 160.0;     //Max yaw rate in deg/sec

float Kp_roll_angle = 0.5;    //Roll P-gain - angle mode
float Ki_roll_angle = 0.05;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.005;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_roll = 0.5;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float pitch_k = 1.2;
float Kp_pitch_angle = Kp_roll_angle*pitch_k;   //Pitch P-gain - angle mode
float Ki_pitch_angle = Ki_roll_angle*pitch_k;   //Pitch I-gain - angle mode
float Kd_pitch_angle = Kd_roll_angle*pitch_k;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
...
float Kd_roll_rate = 0.02;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
float Kp_pitch_rate = 0.15;   //Pitch P-gain - rate mode
float Ki_pitch_rate = 0.2;    //Pitch I-gain - rate mode
float Kd_pitch_rate = 0.02; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)

float Kp_yaw = 0.000;           //Yaw P-gain
float Ki_yaw = 0.0;          //Yaw I-gain
float Kd_yaw = 0.002;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
float GyroErrorX = 0.38;
float GyroErrorY = -4.77;
float GyroErrorZ = -0.65;
...
float s1_command_old = 0 , s2_command_old = 0;
float s1_command_scaled = 0     , s2_command_scaled = 0, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled;
...
void setup() {
  ...
  delay(30);
  servo1.attach(servo1Pin, 850, 2050); //pin, min PWM value, max PWM value
  servo2.attach(servo2Pin, 850, 2050);
...
void controlANGLE() {
  ...
  */
  ...
  error_roll = roll_des-1.0 - roll_IMU;
  error_roll = constrain(error_roll,-5,5);
  ...
  derivative_roll= constrain(derivative_roll,-30,30);
  roll_PID = 0.01 * (Kp_roll_angle * error_roll + Ki_roll_angle * integral_roll - Kd_roll_angle * derivative_roll); //scaled by .01 to bring within -1 to 1 range
  roll_PID = constrain(roll_PID,-2,2);
  //Pitch
  error_pitch = pitch_des - pitch_IMU;
   error_pitch= constrain(error_pitch,-20,20);
  ...
  Serial.print(" ");
  Serial.print(Kd_roll_angle * derivative_roll);
  Serial.print(" ");
  Serial.print(-2);
  Serial.print(" ");
  Serial.print(2);
  ...
  error_yaw = constrain(error_yaw,-20,20);
...
void controlMixer() {
  ...
  */
  ...
  if (channel_3_pwm < 1020)
  {
    ...
    // roll_PID+=0.5;
      m2_command_scaled = thro_des -0.17 + roll_PID + yaw_PID - pitch_PID; //Front Left
      m3_command_scaled = thro_des -0.17 - 0.02-roll_PID - yaw_PID - pitch_PID; //Front Right
      m4_command_scaled = thro_des -0.1  + 0.03 - roll_PID + yaw_PID + pitch_PID; //Back Right
      m1_command_scaled = thro_des -0.15 + 0.02 + roll_PID - yaw_PID + pitch_PID; //Back Left
    ...
    // m3_command_scaled = thro_des-0.1 + pitch_PID *4 ; //- roll_PID + yaw_PID;
    // m4_command_scaled = thro_des ;//+ pitch_PID + roll_PID - yaw_PID;
    // m5_command_scaled = 0;
    // m6_command_scaled = 0;
    m2_command_scaled*=0.91;
    m3_command_scaled*=0.88;
  }
  float servo_fix = 0.5*(channel_5_pwm - 1500.0) / 1000.0;
  //  Serial.println(Kp_yaw*10);
  ...
  s2_command_scaled     =  angleOfServo- yaw_PID/1.5; //
  s1_command_scaled     =  1 - angleOfServo + servo_fix - yaw_PID/1.5;//: he so dieu chinh do lech 2 servo trai phai
  s3_command_scaled = 0;//(channel_1_pwm - 1500.0) / 1000.0 +  roll_PID;
  s4_command_scaled = 0;//1-yaw_PID;//(channel_6_pwm-1000.0)/1000.0;
...
void scaleCommands() {
  ...
  */
  ...
  //Constrain commands to motors within 0-180
  ...
  m1_command_PWM = constrain(m1_command_PWM, 125, 245);
  m2_command_PWM = constrain(m2_command_PWM, 125, 245);
  m3_command_PWM = constrain(m3_command_PWM, 125, 245);
  m4_command_PWM = constrain(m4_command_PWM, 125, 245);
  ...
  s1_command_old = s1_command_PWM;
  s2_command_old = s2_command_PWM;
  s1_command_PWM = s1_command_scaled * 180;
  s2_command_PWM = s2_command_scaled * 180;
  int ds1 = s1_command_PWM-s1_command_old;
  int ds2 = s2_command_PWM-s2_command_old;
  ds1 = constrain(ds1,-2,2);
  ds2 = constrain(ds2,-2,2);
  s1_command_PWM  = s1_command_old + ds1;
  s2_command_PWM  = s2_command_old + ds2;
...
void printPIDoutput() {
  ...
  Serial.print(roll_PID*100);
  Serial.print(F(" pitch_PID: \t"));
  Serial.print(pitch_PID*100);
  Serial.print(F(" yaw_PID: \t"));
  Serial.println(yaw_PID*100);
