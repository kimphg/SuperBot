...
float B_madgwick = 0.06;  //Madgwick filter parameter
float B_accel = 0.14;     //Accelerometer LP filter paramter, (MPU6050 default: 0.14. MPU9250 default: 0.2)
float B_gyro = 0.1;       //Gyro LP filter paramter, (MPU6050 default: 0.1. MPU9250 default: 0.17)
float B_mag = 1.0;        //Magnetometer LP filter parameter

//Magnetometer calibration parameters - if using MPU9250, uncomment calibrateMagnetometer() 
float MagErrorX = 0.0;
float MagErrorY = 0.0; 
float MagErrorZ = 0.0;
float MagScaleX = 1.0;
...
float Kp_roll_rate = 0.0;    //Roll P-gain - rate mode
float Ki_roll_rate = 0.0;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.000;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
...
float Kd_yaw = 0.00;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
void setup() {
  ...
  //Initialize all pins
  pinMode(20, OUTPUT); //Pin 13 LED blinker on board, do not modify 
...
// calibrateESCs();
  ...
  delay(5);
   ...
   //PROPS OFF. Uncomment this to calibrate your ESCs by setting throttle stick to max, powering on, and lowering throttle to zero after the beeps
...
void calibrateESCs() {
   ...
   while (true) {
      ...
      digitalWrite(20, HIGH); //LED on to indicate we are not in main loop
...
void loopBlink() {
  ...
  if (current_time - blink_counter > blink_delay) {
    ...
    digitalWrite(20, blinkAlternate); //Pin 13 is built in LED
...
void printRollPitchYaw() {
  if (current_time - print_counter > 10000) {
    print_counter = micros();
     Serial.print(Ki_pitch_angle*integral_pitch);Serial.print(F(","));
     Serial.print(Kp_pitch_angle*error_pitch);Serial.print(F(","));
     Serial.print(pitch_PID*100.0);Serial.print(F(","));
    Serial.print(Ki_roll_angle*integral_roll);Serial.print(F(","));
     Serial.print(Kp_roll_angle*error_roll);Serial.print(F(","));
      Serial.print(roll_PID*100.0);Serial.print(F(","));
